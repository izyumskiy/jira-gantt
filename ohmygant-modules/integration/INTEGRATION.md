...
`repositoryGateway` использует только явные URL из нормализованного источника: текст Jira/Confluence и Jira remote links. Сопоставление по префиксу ключа, проекту, команде или названию запрещено. Разрешение на чтение запрашивается для фактически найденного домена.
...
Каждая строка `result.workItems[]` возвращает `estimateExplanation`: базовый метод, входы, формулу и сценарии P50/P80/P90. Поле предназначено для UI и аудита арифметики; принимающая оболочка не должна пересчитывать его самостоятельно.

`result.portfolio.backtest` содержит отдельные метрики часов и дат, покрытие P50/P80/P90 и сигнал дрейфа. В выборку допускаются только инициативы, завершившиеся до старта проверяемой инициативы.
